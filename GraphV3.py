...
class Graph():
    ...
    def addNode(self, v):
        self.G.add_node(str(v), dfi=0, visited = False, backEdgeList=[])
        self.V+=1
         ...
         for w in list(self.G.neighbors(oldS)):
            if not self.getDFI(w) in self.palmTree:
                ...
                if self.palmTree.has_edge(self.getDFI(w), s) == False:
                    ...
                    self.G.nodes[w]["backEdgeList"].append((self.getDFI(s), self.getDFI(w)))
    ...
    #i = componenteBiconnessa v = radice
    def creaStrutturaListe(self, i, v, visitati):
        #print("v: ", v)
        #print(type(v))
        i.nodes[v]['visitato'] = True
        lista = [v]
        for w in i.neighbors(v):
            #i.nodes[w]['visitato'] = True
            #print("w: ", w)
            if self.palmTree.nodes[w]['lowpoint']<v:
                 i.nodes[w]['esternamenteAttivo'] = True
            else:
                i.nodes[w]['esternamenteAttivo'] = False
            tmp = [(v, w)]
            lista.append(tmp)
            #print("lista: ", lista)
            if not i.nodes[w].get('visitato', False):
                self.creaStrutturaListe(i, w, visitati)
        
        self.componentiDFS.append(lista) 

    def walkup(self, gt, v, w):
        print("gt: ", gt,"v: ",v,"w: ",w)
        for i in list(nx.connected_components(gt)):
            print(i)
            if v in i:
                print("trovato w: ", i)
                self.creaStrutturaListe(i, next(iter(i)), [])
